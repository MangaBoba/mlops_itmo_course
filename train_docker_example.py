...
def train(config, model, device, train_loader, optimizer, epoch, writer):
    ...
    writer.add_scalar("training acc", epoch_acc, epoch * len(train_loader))
...
def validate(model, device, val_loader, epoch, writer):
    ...
    writer.add_scalar("val loss", epoch_loss, epoch * len(val_loader))
    writer.add_scalar("val acc", epoch_acc, epoch * len(val_loader))
...
def test(model, config: AppConfig):
    ...
    if use_cuda:
        cuda_kwconfig = {"num_workers": 1, "pin_memory": True, "shuffle": True}
    ...
    writer.add_scalar("test loss", epoch_loss, len(test_loader))
    writer.add_scalar("test acc", epoch_acc, len(test_loader))
